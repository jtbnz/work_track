...
if (empty($input['misc_id'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Misc ID is required']);
    exit;
}

if (!isset($input['included'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Included status is required']);
    exit;
}
...
$result = $quoteModel->updateMiscItem($input['quote_id'], $input['misc_id'], $input['included']);
...
if ($result) {
    ...
    echo json_encode(['success' => false, 'message' => 'Failed to update misc item']);
}
